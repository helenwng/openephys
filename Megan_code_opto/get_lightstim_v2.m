...
light_thresh = 20*std(light_out(ex_trial,:))+max(light_out(ex_trial,:));       % considered light trial if max value  exceeds 20 standard deviations of ex_trial above the max value of ex_trial
...
if sum(light_trials(1:min_length)~=light_trials_check(1:min_length))    % if numbers of light trials don't agree
    ...
    h = figure; plot(light_out(light_trials(off_trial),:));
...
if strcmp(exp_type,'ramp')
    for t = 1:length(light_trials)
        ...
        light_dur(t) = 1;      % assumes ramp lasts 1sec
        light_start(t) = light_end(t)-light_dur(t)*1000+1;                     % go backwards to find the start
    end
    light_hz = 1;
...
%     light_dur = round(max(light_dur));              % in case of trials with only single pulse, still consider light duration of trials with multiple pulses
    ...
    light_hz(light_hz>10&light_hz<=11) = 10;
    light_hz(light_hz>20&light_hz<=21) = 20;
    light_hz(light_hz>1&light_hz<=2) = 1;
...
%         light_start(t) = find(light_out(tt,:)>light_thresh,1,'first');
%         light_end(t) = find(light_out(tt,:)>light_thresh,1,'last');
        light_start(t) = find(light_out(tt,:)>(10*std(light_out(tt,1:250))+mean(light_out(tt,1:250))),1,'first');
        light_end(t) = find(light_out(tt,:)>(10*std(light_out(tt,1:250))+mean(light_out(tt,1:250))),1,'last');
        light_dur(t) = length(light_start(t):light_end(t))/1000;
...
%     light_dur = pulse_dur;
    ...
    all_light(light_trials)=1;
            all_light(light_trials(find(light_start<300))) = 2;     % in case of multiple light conditions with different start times
    all_light(light_trials(light_start>300)) = 1;
...
%     end
    ...
    %         find((light_inten == intensities(small_diffs)) | (light_inten == intensities(small_diffs+1)))
        ...
        all_light(light_trials(light_start<300)) = max(all_light)+1;     % in case of multiple light conditions with different start times
...
for c = 2:length(light_conds)       % assuming 0 (no light) is a light condition
    which_trials = ismember(light_trials,find(all_light==light_conds(c)));
    av_light_start(c-1) = time_index(round(mean(light_start(which_trials))));      % average time at which light started (1 less than sample # b/c starting with 0!)
    new_pulse_dur(c-1) = mean(light_dur(which_trials));
end
light_dur = round(mean(light_dur));
pulse_dur = [0 new_pulse_dur]; % assuming 0 (no light) is a light condition
% pulse_dur and all_light already made 

% save parameters
save('light_params.mat','all_light', 'pulse_dur', 'light_dur', 'av_light_start');
