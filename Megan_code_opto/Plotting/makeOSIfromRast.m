function [OSI_off OSI_on DSI_off DSI_on] = makeOSIfromRast(spike_mat,preMS,stimMS,sumPeriod,oridom,trial_num,trial_subset,normalizing)
% this function will generate a OSI LED on vs. off plot from a raster plot
...
assert(ndims(trial_num) ==2,'This code currently only works for 2 dimensions (e.g. ori and LED)'); % if you have two variables (e.g. orientation/LED)
    ...
    dsiDistance = length(oridom)/2; %assumes even number of ori
    osiDistance = dsiDistance/2;
    oridom_rad = degtorad(oridom);
    tuning_curve_rep = repmat(tuning_curve,1,2);        % duplicate the columns of tuning_curve for indexing purposes

    peak_off = find(tuning_curve(1,:)==max(tuning_curve(1,:)));     % get indice of orientation with peak response
    OSI_off = (tuning_curve_rep(1,peak_off)-(tuning_curve_rep(1,peak_off+osiDistance)))/(tuning_curve_rep(1,peak_off)+(tuning_curve_rep(1,peak_off+osiDistance))); %Determine OSI, max - response at orthogonal ori (90 degrees away)
    OSI_CV_off = abs((sum(tuning_curve(1,:).*exp(1i*2*oridom_rad)))/sum(tuning_curve(1,:))); % Determine OSI, 1-CV
    DSI_off = (tuning_curve_rep(1,peak_off)-(tuning_curve_rep(1,peak_off+dsiDistance)))/(tuning_curve_rep(1,peak_off)+(tuning_curve_rep(1,peak_off+dsiDistance)));
    DSI_CV_off = abs((sum(tuning_curve(1,:).*exp(1i*oridom_rad)))/sum(tuning_curve(1,:)));

    peak_on = find(tuning_curve(2,:)==max(tuning_curve(2,:)));     % get indice of orientation with peak response
    OSI_on = (tuning_curve_rep(2,peak_on)-(tuning_curve_rep(2,peak_on+osiDistance)))/(tuning_curve_rep(2,peak_on)+(tuning_curve_rep(2,peak_on+osiDistance))); %Determine OSI, max - response at orthogonal ori (90 degrees away)
    OSI_CV_on = abs((sum(tuning_curve(2,:).*exp(1i*2*oridom_rad)))/sum(tuning_curve(2,:))); % Determine OSI, 1-CV
    DSI_on = (tuning_curve_rep(2,peak_on)-(tuning_curve_rep(2,peak_on+dsiDistance)))/(tuning_curve_rep(2,peak_on)+(tuning_curve_rep(2,peak_on+dsiDistance)));
    DSI_CV_on = abs((sum(tuning_curve(2,:).*exp(1i*oridom_rad)))/sum(tuning_curve(2,:)));
