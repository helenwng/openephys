...
dataTime = dataTime./dataInfo(1).header.sampleRate;
nsamples = length(dataTime);

%load ADC inputs
for i = 1:2,
    [ADCin(i,:),~,ADCinfo(i,:)] = load_open_ephys_data(sprintf('100_ADC%d.continuous',i));
end
...
if epocOn(1)<epocOff(1)
    if size(epocOn)==size(epocOff)
        ...
        trials = [dataTime(epocOn) dataTime(epocOff)];
    end
